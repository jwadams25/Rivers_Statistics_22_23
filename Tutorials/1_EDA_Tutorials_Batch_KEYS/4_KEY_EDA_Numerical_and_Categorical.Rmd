...
**filter**: Think of this fuction like the options you have when you are online shopping that allow you to narrow down the scope of products you'd like to see. You do not always need to use this. We use it in this case because we do not want to include any cells in our numerical variable column that have an NA.
...
3.  What is another visualization that could help you uncover those missing pieces of information?
    ...
    Histogram, Density Curve, Ridge Plot
...
2.  The last line of code is going to do the stacking or **faceting**. You want to **facet it by the categorical** variable and then make the **number of rows** equal to the number of **levels**. If you don't know the number of levels use the code chunk below.
...
3.  Change CATEGORICAL_VARIABLE and NUMBER_OF_LEVELS
...
1.  Did you learn anything new about the distributions by looking at the visualizations you created? If so, explain.
    ...
    I learned more about how many counties were in each group and how they were distributed across varying poverty rates.

2.  What new questions does this information raise?

    Why. might those counties without a metropolitan area tend to have higher rates of poverty?

    If the poverty rates are higher in counties that don't contain a metro area, how might the median income of those counties compare to one another?
...
county |>
  ...
  geom_density_ridges(alpha = 0.5) +
   labs(
    title ="How does the median household income in counties without \na metro area compare to those with a metro area?",
    subtitle = "Counties with a metro area tend to have higher median household incomes.",
    x = "Median Household Income ($) ",
    y = "Whether a county have a metro area") +
